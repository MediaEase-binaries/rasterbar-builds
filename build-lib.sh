...
install_boost() {
    ...
    sudo mkdir -p /opt/boost_${BOOST_VERSION}
    
    # Liens symboliques des en-têtes
    sudo ln -sf /tmp/boost/include/* /opt/boost_${BOOST_VERSION}/
    
    # Liens symboliques des bibliothèques
    sudo ln -sf /tmp/boost/lib/* /opt/boost_${BOOST_VERSION}/
    
    # Liens symboliques des outils de build
    if [ -f "/tmp/boost/bin/b2" ]; then
        echo "====> Found b2 at /tmp/boost/bin/b2, creating symlinks"
        sudo ln -sf /tmp/boost/bin/b2 /opt/boost_${BOOST_VERSION}/b2
    ...
    if [ -f "/tmp/boost/bin/bjam" ]; then
        ...
        sudo ln -sf /tmp/boost/bin/bjam /opt/boost_${BOOST_VERSION}/bjam
    ...
    sudo cp -f /tmp/boost/share/boost-build/boost-build.jam /opt/boost_${BOOST_VERSION}/
    sudo cp -f /tmp/boost/share/boost-build/project-config.jam /opt/boost_${BOOST_VERSION}/
